...
BEGIN
    ...
    BEGIN TRY
        ...
        WITH konto AS (
            SELECT k.konto_id, k.konto_bezeichnung,
                   CASE
        ...
        SELECT
            ROW_NUMBER() OVER (ORDER BY konto_id) AS account_key,
            konto_id,
            konto_bezeichnung,
            account_category,
            CASE account_category
                WHEN 'Erlöse'         THEN 1
                WHEN 'COGS'           THEN 2
                WHEN 'Personalkosten' THEN 3
                ELSE                       4   -- Sachkosten
            END AS account_category_sort,
            CASE
                WHEN konto_id LIKE '4%' THEN 'Umsatz'
                WHEN konto_id LIKE '5%' THEN 'COGS'
                ELSE                         'OpEx'
            END AS pl_line,
            CASE
                WHEN konto_id LIKE '4%' THEN 1  -- Umsatz
                WHEN konto_id LIKE '5%' THEN 2  -- COGS
                ELSE                         3  -- OpEx (Personalkosten + Sachkosten)
            END AS pl_sort,
            CASE WHEN konto_id LIKE '4%' THEN 1 ELSE -1 END AS sign
        FROM konto
        ORDER BY konto_id;
